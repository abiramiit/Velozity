...
async function runTests() {
    ...
    const allAssigned = devTasks.data.every((t: any) => t.assignedDeveloperId === dev1User.id);
    ...
    const taskToUpdate = devTasks.data.find((t: any) => t.status === 'TODO');
    if (taskToUpdate) {
        ...
        const actCreated = updatedTask.data.activityLogs.some((a: any) => a.newStatus === 'IN_PROGRESS');
